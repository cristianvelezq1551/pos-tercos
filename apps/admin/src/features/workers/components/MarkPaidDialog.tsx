...
export function MarkPaidDialog({
  ...
  return (
    <Dialog
      ...
      description={`${workerName} · total ${formatCop(total)}`}
      maxWidth="max-w-md"
      footer={
        <>
    ...
    >
      <div className="space-y-4">
        <FormField
          label="Comprobante (foto/captura de transferencia)"
          required
          hint="JPEG, PNG o WebP. Máx 5 MB."
        >
          <Input type="file" accept="image/jpeg,image/png,image/webp" onChange={onFile} disabled={pending} />
        </FormField>
        {preview ? (
