...
export function StaleShiftGate({ shift }: { shift: Shift }) {
  ...
  return (
    // Mismo patrón que la apertura de turno: el scroll en el contenedor y el
    // centrado por `min-h-full`, para que la tarjeta nunca quede fuera de
    // alcance en pantallas bajas.
        ...
        <div className="w-full max-w-md space-y-5 rounded-2xl border border-warning-border bg-card p-6 text-center shadow-sm">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Quedó una caja abierta del{' '}
              <span className="font-semibold text-foreground">{openedDate}</span>. No puedes vender
