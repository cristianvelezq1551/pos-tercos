...
> ~~salvo **B.4b (abrir caja offline)**, que sigue DIFERIDA a propósito~~ **B.4b IMPLEMENTADA
> el 2026-07-06** (decisión del dueño; ver CLAUDE.md §7.v13) — ya no queda ninguna
...
| 2 | **Caja: abrir offline** | Se **permite abrir caja offline** (caja provisional local) + reconciliar. **✅ IMPLEMENTADA (2026-07-06)**: `POST /shifts/sync-offline-open` idempotente + apertura local en el POS que se drena ANTES que las ventas. Ver CLAUDE.md §7.v13. |
...
| **B.0b.1** ✅ | **SW abre offline**: navegaciones network-first PERO cacheando la última buena → offline sirve la shell `/` cacheada (no `offline.html`). Arregla el bug del `.svg` (antes el SW nunca instalaba). `offline.html` solo si nunca cargó online. Online sin cambios (siempre red primero). | (prod build) Cortar red **y recargar** → el POS abre. Requiere prueba en navegador (el SW solo corre en build de producción). |
| **B.0b.2** | **Datos frescos del cache**: catálogo/disponibilidad/sesión leídos de IndexedDB cuando offline (en vez del HTML horneado). Se hace **junto con B.2** (el ledger), que es donde la diferencia se ve. | El catálogo offline refleja el cache + descuentos del ledger, no un congelado. |
...
| **B.2.1** ✅ | **Venta offline encolada + recibo**: con la red caída, "Cobrar" encola la venta en IndexedDB (`OFF-N`, totales verbatim, pago + `offlineVerified`) e imprime el recibo provisional (`TU TURNO OFF-N` + `RECIBO PROVISIONAL` + `PENDIENTE DE SINCRONIZAR`). Banner offline en el modal; `LastSaleBanner` muestra `OFF-N`. Online intacto. typecheck/lint/build ✓, smoke del recibo ✓, `.exe` regenerado. | Offline: cobrar → queda en cola (IndexedDB), sale el papel con `OFF-N`. |
| **B.2.2a** ✅ | **Backend de disponibilidad compartida**: `evaluateAvailability` extraída a `@pos-tercos/domain` (pura, reusada online+offline) + serialización del grafo + `GET /products/offline-snapshot` (grafo + stock crudo). `products.service` refactorizado para usar la función pura. typecheck/lint ✓; **e2e 39/39** (availability incluida → refactor sin cambios de comportamiento). | El endpoint devuelve el snapshot; la disponibilidad online sigue idéntica. |
| **B.2.2b** ✅ | **Ledger local en el POS**: se cachea el snapshot (`cacheStockSnapshot`); cada venta offline descuenta su consumo (`applyConsumptionForSale` vía `expandRecipe`); `useAvailability` cae a `computeOfflineAvailability` (stock − consumido, misma función pura) cuando la red falla → un preparado se agota offline. typecheck/lint/build ✓. | Offline: tras N ventas, un preparado se marca agotado en el catálogo. |
| **B.3** ✅ | **Motor de sync + endpoint**: al volver online el POS vacía la cola → `POST /sales/sync-offline` (idempotente por `localId`, totales verbatim, `paidAt` backdateado, status `ENTREGADO`, descuenta stock, asigna recibo+turno reales; NO toca KDS/turnero/notificaciones). El provider dispara el drain al recuperar conexión; banner "Sincronizando N…". typecheck/lint/build ✓; **e2e 39/39** (2 casos nuevos: número real + ENTREGADO + `paidAt` backdateado + idempotencia). | Vender offline, reconectar → la venta aparece en backend con número real, `paidAt` correcto y stock descontado. |
| **B.4a** ✅ | **Bloqueo de cierre con cola**: `CloseShiftModal` no deja cerrar si `status==='offline'` (el cierre necesita el backend) o hay ventas offline en cola (si no, el efectivo esperado quedaría mal → descuadre falso). El bloqueo se libera solo al terminar de sincronizar. typecheck/lint/build ✓. | Con cola pendiente o sin red, "Cerrar turno" queda bloqueado con motivo; al sincronizar, se habilita. |
| **B.4b** ✅ HECHA (2026-07-06) | **Caja abrible offline + reconciliación**. Requiere mover el gate de turno (hoy SSR con `redirect`) a render cliente → toca el arranque online (territorio de B.0b) y no es verificable sin navegador. Es edge case (solo si la jornada ARRANCA sin red; el corte mid-jornada ya está cubierto). **Diferida** (2026-05-24) hasta verificar el núcleo en build de producción y/o si el negocio realmente arranca jornadas sin internet. | — |
| **B.5** ✅ | **Bandeja de revisión**: cuando quedan ventas fallidas, la banda pasa a roja "N sin sincronizar — Revisar" → abre la bandeja (`OfflineReviewTray`): lista cada venta (OFF-N, monto, método, razón del fallo) con "Reintentar sincronización" y "Descartar (último recurso)" con confirmación. Destraba el cierre (B.4a) cuando se resuelven las fallidas. typecheck/lint/build ✓. | Forzar un fallo de sync → banda roja + la venta en la bandeja con su razón; reintentar o descartar. |
