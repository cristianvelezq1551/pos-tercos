...
| **B.2.2** | **Ledger de disponibilidad local + datos del cache** (incluye B.0b.2): catálogo/disponibilidad leídos de IndexedDB offline; `expandRecipe` descuenta insumos por venta → un preparado se agota offline igual que online. Requiere endpoint `GET /products/offline-snapshot`. | Offline: tras N ventas, un preparado se marca agotado. |
