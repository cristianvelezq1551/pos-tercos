...
| **B.4b** | **Caja abrible offline + reconciliación**: abrir caja provisional offline (día que arranca sin red) + `POST /shifts/sync-offline` (registra/adopta la caja al volver). Edge case (lo normal es que la caja ya esté abierta antes del corte). | Día que arranca offline: abrir caja, vender, reconectar → caja registrada + ventas asociadas. |
