...
|---|---|---|
| **B.0** | **Cimientos**: app-shell servible offline (SW cache-first + route autenticado con `sessionSnapshot`, sin SSR); `useConnectivity` + heartbeat + banner; wrapper IndexedDB + stores; cache de catálogo/promos/config + **grafo de recetas + snapshot de stock**. | Cortar red **y recargar la pestaña** → el POS abre (no `offline.html`); banner offline; catálogo desde cache. |
